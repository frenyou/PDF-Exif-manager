...
; ===============================================================================
; 注意！Exiftool(截至12.83)对中文支持不佳，命令行读取和输出中文时会乱码。
; 从0.56开始，勾选系统区域设置的“use utf-8 for worldwide language support”
; 不再使用临时文本文件作为中转，不再生成及删除文本文件。
...
; 应用名称
appname1 = PDF元数据管理器_v0.66
appname2 = 禁止复制...%appname1%
...
; 程序名称
CropImageTool := "图像裁剪工具v1.7.exe"   ; 注意fileinstall命令处要同步修改
; 程序路径
...
; 目前改动了：1编辑单元格时，按del键为删除输入文字。不编辑时，为删除所选行。
;            2双击单元格启动编辑，改为单击单元格。
...
FileInstall, 图像裁剪工具v1.7.exe, %CropImageToolPath%
...
Gui,main: +LastFound +Resize hwndGUIMainHD ;AlwaysOnTop
...
Gui, main: Add, Text, % dpi("x+12 ys-2 H16 hwndSquareHD"), ▓▓▓▓▓▓ ;
...
Gui,main: Add, Button,% dpi("x+12 H21 w62 HwndButtonSaveLBHD gButtonSaveLB Section"), 保存更改

; 创建封面栏
Gui, main: Font, % dpi("s8")  ; ↓x+11控制图片和listview间隙，下行y+-5控制listview上边y值
Gui, main: Add, Text,% dpi("x+11 ys+10 h25 w230 hwndButtChangeImageHD Right gButtChangeImageLB"), 更 换 图 片
Gui, main: Add, Picture, % dpi("y+-5 hwndImage2HD h-1 w230 gCropImageLB Section")
GuiControl, Hide, %ButtChangeImageHD%

Gui,main: Font, % dpi("s11")
ButtLastWidth := ((230 * A_ScreenDPI / 96) / 2 - 15 ) / A_ScreenDPI * 96
Gui, main: Add, Text, % dpi("xs y420 w" ButtLastWidth " Right hwndButtLastImageHD gButtLastImageLB"), <<<
...
, 编号|书名|作者|出版日期|丛书|出版商|格式|路径|大小(MB)|页数|制作者|生成工具1|生成工具2|生成日期|修改日期|元数据日期|关键词|描述              ; exif项目相关命令
...
Menu, MyContextMenu, Add, 在资源浏览器中打开, ContextOpenInExplorerLB
Menu, MyContextMenu, Add, 重命名文件, RenameFileLB
...
Menu, MyContextMenu, Add, 保存所有行, ButtonSaveLB
Menu, MyContextMenu, Add,
...
; 确定窗口高度后，再显示底行（状态栏）
Gui,main:Font, % dpi("s11")
Gui,main:Add,Text,% dpi("w194 yp-3 HwndGUIattribute1 ")
;显示选择多少项及显示搜索中
Gui,main:Add,Text,% dpi("w306 x+m HwndGUIattribute2")
...
; 注册热键
Hotkey, Del, ContextClearRowsLB, On
return
...
; 4 标题显示处理提示
WinSetTitle, %appname1%,, %appname2%
GuiControl,,%GUIattribute1%, 已读取共0项
 ...
 if (LoopFileExt = TypeFileExt)
 {
  ...
  Loop, parse, OutputVar, `n, `r
  {
   if A_LoopField contains error
   {
    ...
    MsgBox, 4096,, % FilePath "`n" OutputVar
   }
  }  
...
ClipSaved := BookExifSave := RowIndex1 := RowIndex2 := ""  ; 清空“粘贴元数据历史记录”（不管有无）
...
if ErrorLog
{
 ...
 FormatTime, DateString,, yyyyMMddHHmmss
...
FileNo := LV_GetCount()
GuiControl,,%GUIattribute1%, 已读取共%FileNo%项

WinSetTitle, %appname2%,, %appname1%
return
...
Loop
{
 if !WinActive("ahk_id" GUIMainHD)
  WinActivate, ahk_id %GUIMainHD%
 Else
  break
 Sleep, 200
}
...
Gui, Sub3: +AlwaysOnTop -DPIScale
Gui, Sub3: Font, % dpi("s10"),微软雅黑 
Gui, Sub3: Font, cFF9900
Gui, Sub3: add, Text, % dpi("y25 HwndSub3Text1HD"), 选择需要命名的元数据字段
GuiControlGet, Sub3Text1Pos, Pos, %Sub3Text1HD%
Sub3Text1PosX := ((1644* GuiDPI / 96) - Sub3Text1PosW) / 2 
GuiControl, move, %Sub3Text1HD%, x%Sub3Text1PosX%
Gui, Sub3: Font, % dpi("s8")
Gui, Sub3: add, Text, % dpi("x30 y+30 Section"), 字段1:
Gui, Sub3: add, DropDownList, % dpi("xs w125 vExifRename1Var")
, 无|书名|作者|出版日期|丛书||出版商|页数|制作者|生成工具1|生成工具2|关键词|描述

Gui, Sub3: add, Text, % dpi("ys Section"), 字段2:
Gui, Sub3: add, DropDownList, % dpi("xs w125 vExifRename2Var")
, 无|书名||作者|出版日期|丛书|出版商|页数|制作者|生成工具1|生成工具2|关键词|描述

Gui, Sub3: add, Text, % dpi("ys Section"), 字段3:
Gui, Sub3: add, DropDownList, % dpi("xs w125 vExifRename3Var")
, 无|书名|作者||出版日期|丛书|出版商|页数|制作者|生成工具1|生成工具2|关键词|描述

Gui, Sub3: add, Text, % dpi("ys Section"), 字段4:
Gui, Sub3: add, DropDownList, % dpi("xs w125 vExifRename4Var")
, 无|书名|作者|出版日期|丛书|出版商||页数|制作者|生成工具1|生成工具2|关键词|描述

Gui, Sub3: add, Text, % dpi("ys Section"), 字段5:
Gui, Sub3: add, DropDownList, % dpi("xs w125 vExifRename5Var")
, 无|书名|作者|出版日期||丛书|出版商|页数|制作者|生成工具1|生成工具2|关键词|描述

Gui, Sub3: add, Text, % dpi("ys Section"), 字段6:
Gui, Sub3: add, DropDownList, % dpi("xs w125 vExifRename6Var")
, 无||书名|作者|出版日期|丛书|出版商|页数|制作者|生成工具1|生成工具2|关键词|描述

Gui, Sub3: add, Text, % dpi("ys Section"), 字段7:
Gui, Sub3: add, DropDownList, % dpi("xs w125 vExifRename7Var")
, 无||书名|作者|出版日期|丛书|出版商|页数|制作者|生成工具1|生成工具2|关键词|描述

Gui, Sub3: add, Text, % dpi("ys Section"), 字段8:
Gui, Sub3: add, DropDownList, % dpi("xs w125 vExifRename8Var")
, 无||书名|作者|出版日期|丛书|出版商|页数|制作者|生成工具1|生成工具2|关键词|描述

Gui, Sub3: add, Text, % dpi("ys Section"), 字段9:
Gui, Sub3: add, DropDownList, % dpi("xs w125 vExifRename9Var")
, 无||书名|作者|出版日期|丛书|出版商|页数|制作者|生成工具1|生成工具2|关键词|描述

Gui, Sub3: add, Text, % dpi("ys Section"), 字段10:
Gui, Sub3: add, DropDownList, % dpi("xs w125 vExifRename10Var")
, 无||书名|作者|出版日期|丛书|出版商|页数|制作者|生成工具1|生成工具2|关键词|描述

Gui, Sub3: add, Text, % dpi("ys Section"), 字段11:
Gui, Sub3: add, DropDownList, % dpi("xs w125 vExifRename11Var")
, 无||书名|作者|出版日期|丛书|出版商|页数|制作者|生成工具1|生成工具2|关键词|描述

Gui, Sub3: Font, % dpi("s10")
Gui, Sub3: add, Text, % dpi("x30 y+60"), 出版日期：
...
Gui, Sub3: add, Edit, % dpi("x+10 W90 H38 vSub3EditLinkVar"), _

Gui, Sub3: Font, % dpi("Norm")
Gui, Sub3: add, Text, % dpi("x30"), 重命名模板：
Gui, Sub3: add, button, % dpi("xs yp w125 h33 gSub3GUISubmitLB Section"), 生成字段
Gui, Sub3: add, button, % dpi("xp-145 w125 h33 gSub3GUITestLB"), 测试

Gui, Sub3: Font, % dpi("c99FF33")
Gui, Sub3: add, Edit, % dpi("x30 w1585 h45 HwndEditExifRenameHD vReNamePattern"), %ReNamePattern%

Gui, Sub3: add, button, % dpi("xs y+60 w125 h40 gSub3guiclose Section"), 取消
Gui, Sub3: add, button, % dpi("xp-145 w125 h40 gSub3GUIYesLB"), 确定

; 修改控件颜色，RGB模式
Gui, Sub3: Color,, 202020
Gui, Sub3: Show, % dpi("w1644 h490")
Return
...
MsgBox, 4096,, % "重命名模板为：`n`n" ReNamePattern
gosub, Sub3guiclose
Return
...
ModifyColumnLB:
Gui,main:Default
; 点击ListView时会记录鼠标所在列号，如果列号是1789或10列
...
Sub1Text1PosX := ((320* GuiDPI / 96) - Sub1Text1PosW) / 2 
GuiControl, move, %Sub1Text1HD%, x%Sub1Text1PosX%
Gui, Sub1: add, Text, % dpi("x40"), 请输入查找字符：
Gui, Sub1: Font, c99FF33
Gui, Sub1: add, Edit, % dpi("w238 vSearchVar"), %Clipboard%
Gui, Sub1: Font, cFF9900
Gui, Sub1: add, Text,, 请输入替换字符：
Gui, Sub1: Font, c99FF33
Gui, Sub1: add, Edit, % dpi("w238 vReplaceVar")
Gui, Sub1: add, button, % dpi("x90 y+10 w60 h25 gSub1GUISubmitLB Section"), 确定
Gui, Sub1: add, button, % dpi("x+20 ys w60 h25 gSub1guiclose"), 取消
; 修改控件颜色，RGB模式
Gui, Sub1: Color,, 202020
Gui, Sub1: Show, % dpi("w320 h180")
Return
...
Loop
{
 ...
 if !RowNumber  ; 上面返回零, 所以没有更多选择的行了.
  break
 LV_GetText(ColumnModifyText, RowNumber, ColumnNo)
 ColumnModifyText := StrReplace(ColumnModifyText, SearchVar, ReplaceVar)
 LV_Modify(RowNumber, "Col" ColumnNo, ColumnModifyText)
}
...
Sub2Text1PosX := ((285* GuiDPI / 96) - Sub2Text1PosW) / 2 
GuiControl, move, %Sub2Text1HD%, x%Sub2Text1PosX%
Gui, Sub2: add, Text, % dpi("x83"), 请输入要提取第几页：
Gui, Sub2: Font, c99FF33
Gui, Sub2: add, Edit, % dpi("x117 w50 vPageNoVar number"), 1
Gui, Sub2: add, button, % dpi("x72 w60 h25 gSub2GUISubmitLB Section"), 确定
Gui, Sub2: add, button, % dpi("x+20 ys w60 h25 gSub2guiclose"), 取消
; 修改控件颜色，RGB模式
Gui, Sub2: Color,, 202020
Gui, Sub2: Show, % dpi("w285 h130")
Return
...
生成工具1原文为Producer，生成工具2原文为Creator Tool。`n`n
热键：Del 清除所选行 
`           Ctrl+a 全选行
`           z 复制元数据（仅当启用时） 
`           x 粘贴元数据（仅当启用时）
...
if UnSaveIDN
{
 ...
 IfMsgBox, No
  return
 Else {
 UnSaveIDN := BookExifSave := RowIndex1 := RowIndex2 := ""  ; 清空原来的“粘贴元数据历史记录”
 LV_InCellEdit.Changed.Remove(ListViewHD, "")
 }
} else {
...
;调整「按钮-保存更改」横向位置
ButtonSaveLBHDXminus := 318*GuiDPI/96
GuiControl, Move, %ButtonSaveLBHD%, % " x" (A_GuiWidth - ButtonSaveLBHDXminus)

;调整「补位方块」横向位置
GuiControl, Move, %SquareHD%, % "x" (A_GuiWidth - ButtonSaveLBHDXminus)

;调整「图像1」横向位置
Image1Plus := 25*GuiDPI/96
GuiControl, Move, %Image1HD%, % "x" (A_GuiWidth - ButtonSaveLBHDXminus + Image1Plus)
...
if !CMDcode
{
 ...
 (join&
 cd /d %LoadFolder%
 %ExiftoolPath% %OverWrite% %SubCode% %CmdOutput%
 )
}
...
PublisherList =
(
A 安徽美术出版社||B 北京大学出版社|B 北京大学医学出版社|B 北京工艺美术出版社|B 北京联合出版公司|B 北京邮电大学出版社|C 长春出版社|C 重庆大学出版社|D 第二军医大学出版社|D 第四军医大学出版社|G 高等教育出版社|G 广西美术出版社|G 广西师范大学出版社|G 贵州人民出版社|H 海洋出版社|H 河北美术出版社|H 合記圖書出版社|H 河南科学技术出版社|H 湖南科学技术出版社|H 湖南美术出版社|H 湖南人民出版社|H 化学工业出版社|J 吉林科学技术出版社|J 吉林美术出版社|J 江苏凤凰科学技术出版社|J 江苏科学技术出版社|J 江苏美术出版社|J 江西美术出版社|J 军事医学科学出版社|K 科学出版社|K 科学普及出版社|L 蓝天出版社|L 辽宁科学技术出版社|L 辽宁美术出版社|L 岭南美术出版社|R 人民教育出版社|R 人民军医出版社|R 人民美术出版社|R 人民卫生出版社|R 人民邮电出版社|S 山东画报出版社|S 山东科学技术出版社|S 陕西科学技术出版社|S 商务印书馆|S 上海交通大学出版社|S 上海教育出版社|S 上海科学技术出版社|S 上海科学技术文献出版社|S 上海人民出版社|S 上海人民美术出版社|S 上海三联书店|S 世界图书出版公司|S 四川大学出版社|T 天津科技翻译出版有限公司|T 天津科学技术出版社|T 天津人民出版社|T 天津人民美术出版社|T 天津杨柳青画社|W 文物出版社|X 新星出版社|Y 云南科技出版社|Z 浙江大学出版社|Z 浙江科学技术出版社|Z 浙江人民美术出版社|Z 知识产权出版社|Z 中国长安出版社|Z 中国电力出版社|Z 中国电影出版社|Z 中国建筑工业出版社|Z 中国连环画出版社|Z 中国青年出版社|Z 中国文联出版社|Z 中国协和医科大学出版社|Z 中国医药科技出版社|Z 中国传媒大学出版社|Z 中南大学出版社|Z 中山大学出版社|Z 中信出版社|Z 中央编译出版社|Z 中国水利水电出版社
)
return